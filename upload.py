...
def create_folder_handler(request: HttpRequest, response: HttpResponse):
    if request.method != HttpMethod.POST:
        response.code = HttpStatus.METHOD_NOT_ALLOWED
        return
    ...
    os.mkdir(os.path.join('data', path_param, name))
    response.code = HttpStatus.OK
    response.body = "Folder created successfully."
